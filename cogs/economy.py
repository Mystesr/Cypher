...
class Economy(commands.Cog):
    ...
    async def work(self, interaction: discord.Interaction):
        ...
        if data["last_work"]:
            ...
            if diff < timedelta(hours=WORK_COOLDOWN_HOURS):
                ...
                await interaction.response.send_message(
                    f"😓 You're tired! Rest for **{m} more minute(s)** before working again.",
                    ephemeral=True
                )
                return
    ...
    async def transfer(self, interaction: discord.Interaction, user: discord.Member, amount: int):
        ...
        if sender["balance"] < amount:
            return await interaction.response.send_message(
                f"❌ Insufficient funds! You have {coin(sender['balance'])}.", ephemeral=True
            )

        self.db.update_balance(interaction.user.id, -amount)
        self.db.update_balance(user.id, amount)
        self.db.get_user(user.id, user.display_name)  # ensure target exists
        self.db.log_transaction(interaction.user.id, -amount, "transfer_out", f"Sent to {user.display_name}")
        self.db.log_transaction(user.id, amount, "transfer_in", f"Received from {interaction.user.display_name}")
    ...
    # ── /lend ──────────────────────────────────────────────
    @app_commands.command(name="lend", description="Lend coins to a friend as a debt")
    ...
    async def lend(self, interaction: discord.Interaction, user: discord.Member, amount: int):
        if user.id == interaction.user.id:
            return await interaction.response.send_message("❌ Can't lend to yourself!", ephemeral=True)
        if amount <= 0:
            return await interaction.response.send_message("❌ Amount must be positive!", ephemeral=True)
        ...
        if lender["balance"] < amount:
            return await interaction.response.send_message(
                f"❌ You only have {coin(lender['balance'])}!", ephemeral=True
            )

        self.db.get_user(user.id, user.display_name)  # ensure borrower exists
        self.db.update_balance(interaction.user.id, -amount)
        self.db.update_balance(user.id, amount)
        ...
        embed.set_footer(text=f"Borrower: use /paydebt {debt_id} to repay")
        await interaction.response.send_message(embed=embed)

    # ── /paydebt ───────────────────────────────────────────
    @app_commands.command(name="paydebt", description="Repay a debt by ID")
    @app_commands.describe(debt_id="The debt ID to repay (from /debts)")
    async def paydebt(self, interaction: discord.Interaction, debt_id: int):
        ...
        if debt["paid"]:
            return await interaction.response.send_message("✅ This debt is already paid!", ephemeral=True)
        if debt["borrower_id"] != interaction.user.id:
            return await interaction.response.send_message("❌ That's not your debt!", ephemeral=True)
        ...
        if borrower["balance"] < amount:
            return await interaction.response.send_message(
                f"❌ You need {coin(amount)} but only have {coin(borrower['balance'])}!", ephemeral=True
            )

        self.db.update_balance(interaction.user.id, -amount)
        self.db.update_balance(debt["lender_id"], amount)
        self.db.pay_debt(debt_id)
        self.db.log_transaction(interaction.user.id, -amount, "debt_paid", f"Repaid debt #{debt_id}")
    ...
    async def debts(self, interaction: discord.Interaction):
        ...
        if owed_to_me:
            ...
            for d in owed_to_me:
                ...
                lines.append(f"• **{name}** owes {coin(d['amount'])} `(#{ d['id'] })`")
            embed.add_field(name="💵 Owed TO You", value="\n".join(lines), inline=False)
        else:
            embed.add_field(name="💵 Owed TO You", value="*Nobody owes you anything*", inline=False)
        ...
        if i_owe:
            ...
            for d in i_owe:
                ...
                lines.append(f"• You owe **{name}** {coin(d['amount'])} `(#{d['id']})` — `/paydebt {d['id']}`")
            embed.add_field(name="💸 You OWE", value="\n".join(lines), inline=False)
        else:
            embed.add_field(name="💸 You OWE", value="*You're debt-free!* 🎉", inline=False)
    ...
    async def leaderboard(self, interaction: discord.Interaction):
        ...
        for i, row in enumerate(rows):
            ...
            lines.append(f"{medals[i]} **{name}** — {coin(row['balance'])}")
